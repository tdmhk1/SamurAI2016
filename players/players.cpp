...
void SamuraiUndo::apply() {
	si->curX = x; si->curY = y; si->hidden = hidden;
}

void Undo::recField(int* sect) {
	fieldUndo.emplace_front(sect);
}
void Undo::recSamurai(SamuraiInfo* si) {
	samuraiUndo.emplace_front(si);
}
void Undo::apply() {
